...
corruptedMemoryScanner(
  "xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))"
);

function corruptedMemoryScanner(corruptedMemory) {
  const regex = /mul\((\d+),(\d+)\)/g;

  const matches = corruptedMemory.matchAll(regex);
  let totalSum = 0;

  for (const match of matches) {
    console.log(match);
    let mul = match[1] * match[2];
    totalSum += mul;
  }
...
function corruptedMemoryScannerWithDoAndDont(corruptedMemory) {
  const regex = /mul\((\d+),(\d+)\)/g;
